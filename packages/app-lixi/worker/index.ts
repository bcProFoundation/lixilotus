...
self.addEventListener('push', event => {
  const promiseChain = getFocusedWindow().then(async focusedWindow => {
    const notification = event.data.json();
    const { message, url, title, icon } = notification;
    const webpushTitle = title ?? 'Notification';
    const options = {
      body: message,
    ...
    if (!focusedWindow) {
      registration.showNotification(webpushTitle, options);
    }
  });
