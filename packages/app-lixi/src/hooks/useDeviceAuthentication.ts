...
export type DeviceAuthenticationValue = {
  isAuthenticationRequired: boolean;
  credentialId: string;
  isSignedIn: boolean;
  loading: boolean;
  turnOnAuthentication: () => void;
  turnOffAuthentication: () => void;
  signUp: () => Promise<void>;
  signIn: () => Promise<void>;
  signOut: () => void;
};
    ...
    signIn: async () => {
      try {
        ...
        if (assertion) {
          ...
          const base64Id = convertArrayBufferToBase64((assertion as any).rawId);
...
export default useDeviceAuthentication;
