...
export function useInfiniteOpenPageMessageSessionByAccountId(
  params: PageMessageSessionListParams,
  fetchAll: boolean = false // if `true`: auto do next fetches to get all notes at once
) {
  const baseResult = useOpenPageMessageSessionByAccountIdQuery(params); //dont query when it is not the owner

  const [trigger, nextResult] = useLazyOpenPageMessageSessionByAccountIdQuery();
  ...
  // Base result
  useEffect(() => {
    next.current = baseResult.data?.allOpenPageMessageSessionByAccountId?.pageInfo?.endCursor;
    if (baseResult?.data?.allOpenPageMessageSessionByAccountId) {
      isBaseReady.current = true;

      const adapterSetAll = pageMessageSessionAdapter.setAll(
        combinedData,
        baseResult.data.allOpenPageMessageSessionByAccountId.edges.map(item => item.node)
      );
  ...
  return {
    data: data ?? [],
    totalCount: baseResult?.data?.allOpenPageMessageSessionByAccountId?.totalCount ?? 0,
