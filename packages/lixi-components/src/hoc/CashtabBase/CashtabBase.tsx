...
import { fiatToSatoshis, adjustAmount, getAddressUnconfirmed, getTokenInfo } from '../../utils/cashtab-helpers';
  ...
  return class extends React.Component<CashtabBaseProps, IState> {
    ...
    handleClick = () => {
        ...
        return window.postMessage(
          {
            ...
            txInfo: {
              address: to,
              value: satoshis ? parseFloat((satoshis! * 10 ** (-1 * Ticker.coinDecimals)).toFixed(2)) : amount
            }
          },
          '*'
        );
      ...
      const sendParams: sendParamsArr = {
        to,
        protocol: coinType,
        value: amount?.toString() || adjustAmount(satoshis, Ticker.coinDecimals, true)
      };
    ...
    setupSatoshisFiat = () => {
      ...
      const intervalPriceNext = setInterval(() => this.updateSatoshisFiat(), PRICE_UPDATE_INTERVAL);
    ...
    componentWillUnmount() {
      const { intervalPrice, intervalUnconfirmed, intervalTimer } = this.state;
      intervalPrice && clearInterval(intervalPrice);
      intervalUnconfirmed && clearInterval(intervalUnconfirmed);
    ...
    componentDidUpdate(prevProps: CashtabBaseProps, prevState: IState) {
      if (typeof window !== 'undefined') {
        const { currency, price, isRepeatable, watchAddress } = this.props;
    ...
    render() {
      ...
      const shouldShowQR = showQR && coinType === Ticker.coinSymbol && (!opReturn || !opReturn.length);
