...
const getTokenInfo = async (coinId: string): Promise<any> => {
  const tokenInfoRequest = await fetch(`https://rest.bitcoin.com/v2/slp/list/${coinId}`);
...
const formatAmount = (amount?: number, decimals?: number): string => {
  ...
  const formatForLargeNum = parseFloat(adjustDecimals).toLocaleString(undefined, {
...
const fiatToSatoshis = async (currency: CurrencyCode, price: number): Promise<number> => {
...
const bchToFiat = async (currency: CurrencyCode, price: number): Promise<number> => {
  const priceRequest = await fetch(buildPriceEndpoint(currency));
  const result = await priceRequest.json();
...
const adjustAmount = (amount?: number, decimals?: number, fromSatoshis?: boolean): string | undefined => {
  decimals = decimals || 0;
